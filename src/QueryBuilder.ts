...
import {
  ...
  Query,
  ReverseComparisonTypes,
  Comparison,
} from './contracts/QueryBuilderContracts';
import { ModelAttributes } from './contracts/ModelContracts';
import {
  OpaqueRow,
  SyncReadAdapterContract,
  SyncWriteAdapterContract,
} from './contracts/AdapterContracts';

export class QueryBuilder<
  Model extends (new () => OpaqueModel) & typeof OpaqueModel
> implements QueryBuilderContract<InstanceType<Model>> {
  constructor(
    public model: Model,
    public $query: Query<ModelAttributes<InstanceType<Model>>> = {}
  ) {}
  ...
  where<
    ...
    Key extends keyof ReverseComparisonTypes<Attributes[Attribute]>
  >(
    attribute: Attribute,
    operator: Key,
    value: ReverseComparisonTypes<Attributes[Attribute]>[Key]
  ) {
    const comparatorKey = Object.entries(Comparison).find(
      ([_, literal]) => literal == operator
    )![0];
    const serialized =
      operator == Comparison._in
        ? (value as any[]).map((value) =>
            this.model.$serializeAttribute(attribute as string, value)
          )
        : this.model.$serializeAttribute(attribute as string, value);
    return this.subQuery({
      ...this.$query,
      [attribute]: { [comparatorKey]: serialized },
    });
  }
  ...
  getSync(
    this: QueryBuilder<Model & { adapter: () => SyncReadAdapterContract }>
  ) {
    return this.$hydrate(this.model.$getAdapter().readSync(this.$query));
  }
  updateSync(
    this: QueryBuilder<Model & { adapter: () => SyncWriteAdapterContract }>,
    data: OpaqueRow
  ) {
    return this.$hydrate(
      this.model.$getAdapter().updateSync(this.$query, data)
    );
  }
  deleteSync(
    this: QueryBuilder<Model & { adapter: () => SyncWriteAdapterContract }>
