import { attribute, OpaqueModel, boot } from '../Model';
import { NoOpAdapter } from '../Adapter';
import { ModelAttributes } from '../contracts/ModelContracts';

const expectAttribute = <M extends TestModel>(model: M) => <
  T extends NonNullable<keyof ModelAttributes<M>>
>(
  attribute: T,
  value: M[T]
) => {
...
describe('attributes', () => {
  ...
  class Model extends TestModel {
    ...
    @attribute({
      serialize: (date?: Date & { ignore?: boolean }) =>
        date?.ignore ? undefined : date?.toISOString(),
      deserialize: (date: unknown) =>
  ...
  test('serialize', () => {
    const test = Model.$fromRow({
      serializing: '2020-12-24T00:00:00',
      attribute: 'default',
      undefinedAttribute: 'lel',
    });
    ...
    expect(Model.$serializeAttribute('serializing', date)).toBe(
      date.toISOString()
    );
    expect(Model.$deserializeAttribute('serializing', 12)).toBe(undefined);
    expect(Model.$serializeAttribute('serializing', ignoreDate)).toBe(
      undefined
    );
  });
  ...
  test('schema inheritation', () => {
    @boot()
    class Parent extends TestModel {
      @attribute()
      public title: string = '';
    }
    @boot()
    ...
    expect(Children.$schema.keys()).toContain('title');
  });
});
...
describe('adapter', () => {
  ...
  test('save and update', async () => {
    ...
    Model.$adapter.create = async () => ({
      title: 'fromcreate',
      id: 'lel',
      describtion: 'fromcreate',
    });
    Model.$adapter.update = async () => [
      { title: 'fromupdate', id: 'lel', describtion: 'fromupdate' },
    ];
